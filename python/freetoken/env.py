...
class EnvClassSingleton:
    ...
    ONECCL_MAX_BUFFER_SIZE = EnvMem(1024**3)
    MAMBA_SSM_DTYPE = EnvStr("float32")
    # Prefer Xe2 XMX tensor cores for GEMM when IPEX / oneDNN exposes them.
    USE_XMX = EnvBool(True)
    # Persist SYCL / Triton-Intel JIT artifacts across runs.
    PERSISTENT_KERNEL_CACHE = EnvBool(True)
