"""ft launch — wire a coding agent (Codex / Claude Code / OpenCode / ...) to a server.

Upstream NVIDIA path: ``python/freetoken/launch.py`` (ported faithfully for the Intel
Arc Pro B70 build; GitHub issue ``agent-launch``).

The module deliberately imports nothing model- or accelerator-related: it resolves a
running ``ft serve`` over HTTP, writes the agent's provider config (or previews it under
``--dry-run``), optionally installs the agent CLI, and points the agent at the server.
That is pure config/CLI plumbing, so it works on a CPU box with no accelerator and no
loaded model — ``--install-only`` needs neither a server nor a model. The two agents that
edit a YAML config (Hermes, dsh) import ``yaml`` lazily inside their preparer so importing
this module — and running every other preparer — stays clean without PyYAML present.
"""
...
def discover_server_model(server: ServerURL) -> ServedModel:
    # The preflight GET on the base URL only confirms the server is reachable; the Intel
    # server exposes /v1/models (and /v1/{chat,responses}) but no bare /v1 route, so a 404
    # here means "up, keep going" rather than a connection failure. Other HTTP errors
    # (4xx/5xx on a route that does exist) and transport errors still abort.
    try:
        try:
...
def prepare_hermes(ctx: LaunchContext) -> CommandSpec:
    ...
    if not ctx.dry_run:
        try:
            import yaml  # PyYAML ships transitively via transformers; only needed on this path
        except ImportError as exc:
            raise RuntimeError(
                "configuring Hermes requires PyYAML (usually installed with transformers)"
            ) from exc
...
def prepare_dsh(ctx: LaunchContext) -> CommandSpec:
    ...
    if not ctx.dry_run:
        try:
            ...
            raise RuntimeError(
                "configuring dsh requires PyYAML (usually installed with transformers)"
            ) from exc
...
def parse_argv(argv: list[str], prog: str = "ft launch") -> argparse.Namespace:
    if "--" in argv:
...
def main(argv: list[str] | None = None, prog: str = "ft launch") -> int:
