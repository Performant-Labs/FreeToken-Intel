...
this page for the repo's checks and the WHY behind each deliberate
deviations from it.
...
the job-local named section — the implementation will document which)
and the check greps for `#include <sycl/sycl.hpp>` vs a `sycl_ext::`
